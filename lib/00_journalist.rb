...
def total_de_larray(list)
list.length
    puts list.length
end


def handle_la_plus_courte(little)
    min = little[0].length
little.each do |handle|
    
    if handle.length < min
        min = handle.length
     end
end
puts min
end
def handle_5caract(five)
    number = 0
    five.each do |handle|
if handle.length == 5 
    number = number + 1 
...
puts "#{number} est le nombre de handle contenant 5caractères" 
end

def handle_caps(maj)
    number = 0
    maj.each do |handle|
 if handle[1] == handle.upcase[1]
    number = number + 1
 end
    end
    puts number
...
def reorga(croissant)
     puts croissant.sort_by!{|x| x.length}
    end

def trouver_spencer(array)
    string = "@epenser"
    array.each do |handle|
   if handle == string
     puts array.find_index (handle)
   end
    end
end
